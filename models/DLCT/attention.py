...
class ScaledDotProductWithBoxAttention(nn.Module):
    ...
    def forward(self, queries, keys, values, box_relation_embed_matrix, attention_mask=None, attention_weights=None, hazard_mask=None):
        '''
        Computes
        :param queries: Queries (b_s, nq, d_model)
        :param keys: Keys (b_s, nk, d_model)
        ...
        # Safe-DLCT: Hazard Bias
        if hazard_mask is not None:
             lambda_hazard = 2.0
             w_mn = w_mn + (hazard_mask * lambda_hazard)
...
class MultiHeadBoxAttention(Module):
    ...
    def forward(self, queries, keys, values,box_relation_embed_matrix, attention_mask=None, attention_weights=None, hazard_mask=None):
        ...
        if self.identity_map_reordering:
            ...
            out = self.attention(q_norm, k_norm, v_norm,box_relation_embed_matrix, attention_mask, attention_weights, hazard_mask)
            out = queries + self.dropout(torch.relu(out))
        else:
            out = self.attention(queries, keys, values,box_relation_embed_matrix, attention_mask, attention_weights, hazard_mask)
