...
class LCCA(nn.Module):
    ...
    def forward(self, queries, keys, values, relative_geometry_weights, attention_mask=None, attention_weights=None,
                pos_source=None, pos_cross=None, hazard_mask=None):
        # print('-' * 50)
        # print('layer input')
        # print(queries[11])
        q = queries + pos_source
        k = keys + pos_cross
        att = self.mhatt(q, k, values, relative_geometry_weights, attention_mask, attention_weights, hazard_mask=hazard_mask)
        # print('mhatt outpout')
        # print(att[11])
        att = self.lnorm(queries + self.dropout(att))
        # print('norm out')
...
class MultiLevelEncoder(nn.Module):
    ...
    def forward(self, regions, grids, boxes, aligns, attention_weights=None, region_embed=None, grid_embed=None, classes=None):
        ...
        # Safe-DLCT: Compute Hazard Mask
        hazard_mask = None
        if classes is not None:
            # HAZARD_SET: Person(1), Bicycle(2), Car(3), Motorcycle(4), Bus(6), Truck(8), Traffic Light(10), Stop Sign(13)
            HAZARD_SET = [1, 2, 3, 4, 6, 8, 10, 13]
            device = classes.device
            hazard_tensor = torch.tensor(HAZARD_SET, device=device)
            is_hazard = torch.isin(classes, hazard_tensor) # (b_s, n_regions)
            # Broadcast to attention shape (b_s, 1, 1, n_regions)
            # But wait, grid2region keys are "out_all" which is concat(region, grid).
            # So keys have length n_regions + n_grids.
            # Hazard mask should match key length.
            # Regions are first n_regions. Grids are next n_grids.
            # Grids are not hazards (or we don't know).
            
            hazard_mask_regions = is_hazard.float()
            hazard_mask_grids = torch.zeros((bs, n_grids), device=device)
            hazard_mask_all = torch.cat([hazard_mask_regions, hazard_mask_grids], dim=1) # (b_s, n_regions + n_grids)
            
            hazard_mask = hazard_mask_all.unsqueeze(1).unsqueeze(1) # (b_s, 1, 1, n_total)
        ...
        for l_region, l_grid, l_r2g, l_g2r in zip(self.layers_region, self.layers_grid, self.region2grid,
            ...
            out_grid = l_g2r(out_grid, out_all, out_all, grid2all, grid_aligns,
                             attention_weights, pos_source=grid_embed, pos_cross=pos_cross, hazard_mask=hazard_mask)
...
class TransformerEncoder(MultiLevelEncoder):
    def __init__(self, N, padding_idx, d_in=2048, d_in_region=None, **kwargs):
        super(TransformerEncoder, self).__init__(N, padding_idx, **kwargs)
        if d_in_region is None:
            d_in_region = d_in
            
        self.fc_region = nn.Linear(d_in_region, self.d_model)
    ...
    def forward(self, regions, grids, boxes, aligns, attention_weights=None, region_embed=None, grid_embed=None, classes=None):
        ...
        return super(TransformerEncoder, self).forward(out_region, out_grid, boxes, aligns,
                                                       ...
                                                       region_embed=region_embed, grid_embed=grid_embed, classes=classes)
