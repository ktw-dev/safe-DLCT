...
def predict_captions(model, dataloader, text_field,cider,args):
    ...
    with tqdm(desc='Evaluation', unit='it', total=len(dataloader)) as pbar:
        for it, ((detections, boxes, grids, masks, classes, depth_regions, depth_grids), caps_gt) in enumerate(iter(dataloader)):
            ...
            with torch.no_grad():
                out, _ = model.beam_search(detections, 20, text_field.vocab.stoi['<eos>'], args.beam_size, out_size=1,**{'boxes': boxes, 'grids': grids, 'masks': masks, 'classes': classes})
